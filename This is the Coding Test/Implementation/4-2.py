...
구현 문제 유형은 그리디에서 공부한 그리디 알고리즘 문제 유형과 비교해 큰 차이가 없다.
애초에 구현 유형과 그리디 유형은 별개가 아니라 하나의 문제에 구현 유형과 그리디 유형이
함께 포함된 형태로 출제되는 경우가 많기 때문이다. 그리디 배울 때 언급 했듯이
하나의 문제에는 여러 개의 문제 유형이 포함되는 경우가 많다.
'''
# 내가 푼 코드
n = int(input())
